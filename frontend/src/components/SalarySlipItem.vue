<template>
	<ListItem>
		<template #left>
			<SalaryIcon class="h-5 w-5 hrms-text-muted" />
			<div class="flex flex-col items-start gap-1.5">
				<div class="text-base font-normal hrms-text">
					{{ title }}
				</div>
				<div v-if="doc?.gross_pay" class="text-xs font-normal hrms-text-muted">
					<span>
						{{
							__("{0}: {1}", [
								__("Gross Pay"),
		...
		<template #right>
			<span v-if="doc?.net_pay" class="rounded text-base font-normal hrms-text">
